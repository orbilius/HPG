...
public class HousePriceInfoTest extends TestCase {
	...
	public void testJSonStringAndParsing() {
		 ...
		 * 
		 * String string = new
		 * HousePriceJSONServerParserUtil().housePriceToJSONString(info);
		 * HousePriceInfo parsed = parser.parseHousePriceJson(string);
		 * assertNotNull(parsed); assertEquals(info.getAddress(),
		 * parsed.getAddress()); assertEquals(info.getStartPrice(),
		 * parsed.getStartPrice()); assertEquals(info.getSquareMeters(),
		 * parsed.getSquareMeters()); assertEquals(info.getUrl(),
		 * parsed.getUrl());
		 */
	}
